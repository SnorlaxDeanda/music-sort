...
# Immediate feedback so Launch Services knows the app started.
notify "Starting…"

# Log setup quietly; keep abort dialogs working.
exec >>"$LOG_FILE" 2>&1
echo "---- $(date) ----"
echo "Arch: $(uname -m)"
...
# Prefer extracted runtime in Application Support — more reliable after Gatekeeper
# approval than executing binaries from inside a just-allowed .app bundle.
if [[ -x "$EXTRACTED_PYTHON" && -f "$EXTRACTED_DIR/version" && "$(cat "$EXTRACTED_DIR/version")" == "$RUNTIME_VERSION" ]] \
  && runtime_ok "$EXTRACTED_PYTHON"; then
  PYTHON_BIN="$EXTRACTED_PYTHON"
  PYTHON_HOME="$EXTRACTED_DIR/python"
  echo "Using extracted runtime"
elif [[ -x "$BUNDLED_PYTHON" ]] && runtime_ok "$BUNDLED_PYTHON"; then
  PYTHON_BIN="$BUNDLED_PYTHON"
  PYTHON_HOME="$BUNDLED_DIR/python"
  echo "Using in-bundle runtime"
else
  prepare_extracted
  # Export libs before the Tk smoke test in runtime_ok.
  export PYTHONHOME="$EXTRACTED_DIR/python"
  export PYTHONNOUSERSITE=1
  if [[ -d "$PYTHONHOME/lib/tcl9.0" ]]; then export TCL_LIBRARY="$PYTHONHOME/lib/tcl9.0"; fi
  if [[ -d "$PYTHONHOME/lib/tk9.0" ]]; then export TK_LIBRARY="$PYTHONHOME/lib/tk9.0"; fi
  runtime_ok "$EXTRACTED_PYTHON" || abort "The built-in runtime could not open a window.

Open the launch log for details:
$LOG_FILE"
  PYTHON_BIN="$EXTRACTED_PYTHON"
  PYTHON_HOME="$EXTRACTED_DIR/python"
fi
...
# Replace this shell process with Python so macOS treats Python/Tk as the app.
exec "$PYTHON_BIN" "$RESOURCES/launch_gui.py"
